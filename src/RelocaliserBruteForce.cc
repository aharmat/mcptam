...
#include <mcptam/RelocaliserBruteForce.h>
#include <mcptam/KeyFrame.h>
#include <mcptam/Map.h>

template<int N>
TooN::Vector<N> combine(TooN::Vector<N> vNFirst, TooN::Vector<N> vNSecond, double (*function)(double,double))
{
  TooN::Vector<N> vNResult;
  for(int i=0; i < N; ++i)
  {
    vNResult[i] = function(vNFirst[i], vNSecond[i]);
  }
  
  return vNResult;
}

RelocaliserBruteForce::RelocaliserBruteForce(Map &map, TooN::Vector<6> v6StepSizes, TooN::Vector<6> v6BoundsPadding)
: mMap(map)
{
  // Compute bounds on the variable space by looking at the map data
  // First, find mean pose
  
  TooN::SE3<> se3Mean = GetMeanPose();
  TooN::SE3<> se3MeanInv = se3Mean.inverse();
  
  TooN::Vector<6> v6LowerBound = se3Mean.ln();
  TooN::Vector<6> v6UpperBound = se3Mean.ln();
  ...
  for(MultiKeyFramePtrList::iterator mkf_it = mMap.mlpMultiKeyFrames.begin(); mkf_it != mMap.mlpMultiKeyFrames.end(); ++mkf_it)
  {
    ...
    TooN::SE3<> se3Diff = mkf.mse3BaseFromWorld * se3MeanInv;
    TooN::Vector<6> v6Diff = se3Diff.ln();
    
    v6LowerBound = combine(v6LowerBound, v6Diff, &std::min);
    v6UpperBound = combine(v6UpperBound, v6Diff, &std::max);
  }
  
  v6LowerBound -= v6BoundsPadding;
  v6UpperBound += v6BoundsPadding;
  
  TooN::Vector<6> v6Extents = v6UpperBound - v6LowerBound;
  
  for(int i=0; i < 6; ++i)
  {
    mv6VolumeIdxSize[i] = std::ceil(v6Extents[i] / v6StepSizes[i]);
    if(mv6VolumeIdxSize[i] % 2 == 0)  // make sure it's odd
      mv6VolumeIdxSize[i] += 1;
      
    mv6StepSizes[i] = v6Extents[i] / mv6VolumeIdxSize[i];
  }
  
  mv6Center = v6LowerBound + v6Extents / 2;
  
  // Starting point
  mnDistFromCenter = 0;
  GatherIndices(mnDistFromCenter, mvWorkingIndices);
}

TooN::SE3<> RelocaliserBruteForce::NextPoseGuess()
{
  if(mvWorkingIndices.empty())
  {
    mnDistFromCenter += 1;
    GatherIndices();
  }
...
void RelocaliserBruteForce::GatherIndices()
{
  // The r/p/y coordinates aren't roll, pitch, yaw, they're just components of
  // the flat vector representation of orientation, but I needed some index
  // variables and they seemed to fit
  
  ROS_ASSERT(mvWorkingIndices.empty());
  
  for(int x = -mnDistFromCenter; x <= mnDistFromCenter; ++x)
  {
    if(x > abs(mv6VolumeIdxSize[0]))
      continue;
      
    for(int y = -mnDistFromCenter; y <= mnDistFromCenter; ++y)
    {
      if(y > abs(mv6VolumeIdxSize[1]))
        continue;
      
      for(int z = -mnDistFromCenter; z <= mnDistFromCenter; ++z)
      {
        if(z > abs(mv6VolumeIdxSize[2]))
          continue;
      
        for(int r = -mnDistFromCenter; r <= mnDistFromCenter; ++r)
        {
          if(r > abs(mv6VolumeIdxSize[3]))
            continue;
      
          for(int p = -mnDistFromCenter; p <= mnDistFromCenter; ++p)
          {
            if(p > abs(mv6VolumeIdxSize[4]))
              continue;
      
            for(int y = -mnDistFromCenter; y <= mnDistFromCenter; ++y)
            {
              if(y > abs(mv6VolumeIdxSize[5]))
                continue;
              
              mvWorkingIndices.push_back(TooN::makeVector(x,y,z,r,p,y));
            }
          }
        }
      }
  ...
  std::random_shuffle(mvWorkingIndices.begin(), mvWorkingIndices.end());
}

TooN::SE3<> RelocaliserBruteForce::GetMeanPose()
{
  TooN::SE3<> se3Mean;  // start with identity pose
  ...
  while(1)
  {
    ...
    for(MultiKeyFramePtrList::iterator mkf_it = mMap.mlpMultiKeyFrames.begin(); mkf_it != mMap.mlpMultiKeyFrames.end(); ++mkf_it)
    {
      ...
      n++;        
      r += (se3Mean.get_rotation().inverse() * mkf.mse3BaseFromWorld.get_rotation()).ln(); 
    }
  ...
  for(MultiKeyFramePtrList::iterator mkf_it = mMap.mlpMultiKeyFrames.begin(); mkf_it != mMap.mlpMultiKeyFrames.end(); ++mkf_it)
  {
    ...
    n++;
    v3Trans +=  mkf.mse3BaseFromWorld.get_translation(); 
  }
...
void RelocaliserBruteForce::Init()
{
...
void RelocaliserBruteForce::InitFromMap()
{
  std::cout<<"================================================"<<std::endl;
  std::cout<<"======= RelocaliserBruteForce::InitFromMap ======="<<std::endl;
  std::cout<<"================================================"<<std::endl;
...
void RelocaliserBruteForce::InitFromFiles()
{
  std::cout<<"================================================"<<std::endl;
  std::cout<<"======= RelocaliserBruteForce::InitFromFiles ======="<<std::endl;
  std::cout<<"================================================"<<std::endl;
...
RelocaliserBruteForce::~RelocaliserBruteForce()
{
  delete mpFabMap;
}

void RelocaliserBruteForce::Reset()
{
  delete mpFabMap;
...
TooN::SE3<> RelocaliserBruteForce::BestPose()
{
  return mse3Best;
}

void RelocaliserBruteForce::Add(MultiKeyFrame &mkfCurrent)
{
...
void RelocaliserBruteForce::ComputeBoW(Level& level)
{
  /*
...
void RelocaliserBruteForce::ComputeFinalMatchDescriptors(KeyFrame& kf, cv::Mat& matDescriptors, std::vector<TooN::Vector<2> >& vRootPos)
{
  kf.MakeExtractor();
...
void RelocaliserBruteForce::GatherMeasurementDescriptors(KeyFrame& kf, cv::Mat& matDescriptors, std::vector<MapPoint*>& vpMapPoints)
{
  cv::Mat matTemp;
  int nInsertRow = 0;
...
double RelocaliserBruteForce::EvaluateMatch(KeyFrame& kfSource, cv::Mat& matSourceDescriptors, std::vector<TooN::Vector<2> >& vSourceRootPos, KeyFrame& kfTarget, CVD::ImageRef irTargetOffset)
{
  ...
  if((int)vFinalMatches.size() < RelocaliserBruteForce::snMinFinalMatches)
  {
    std::cout<<"Not enough, minimum is "<< RelocaliserBruteForce::snMinFinalMatches<<std::endl;
...
bool RelocaliserBruteForce::FindBestPose(MultiKeyFrame &mkfCurrent, bool bDraw)
{
  if(nFabMapSize == 0)
    return false;
  ...
  for(KeyFramePtrMap::iterator kf_it = mkfCurrent.mmpKeyFrames.begin(); kf_it != mkfCurrent.mmpKeyFrames.end(); ++kf_it)
  {
    ...
    for(unsigned i=0; i < vFabMapMatches.size(); ++i)
    {
      ...
      if(match.match < RelocaliserBruteForce::sdMinLoopProbability)  // Match probability too low
        continue;
...
double RelocaliserBruteForce::OptimizePose(KeyFrame& kfSrc, const KeyFrame& kfTarget, const std::vector<TooN::Vector<2> >& vSourceRootPos, const std::vector<MapPoint*>& vpMapPoints, const std::vector<cv::DMatch>& vMatches)
{
  ...
  for(int nR = 0; nR < RelocaliserBruteForce::snNumFitTrials ; nR++)
  { 
...
TooN::Vector<6> RelocaliserBruteForce::PoseUpdateStep(KeyFrame& kf, TrackerDataPtrVector& vTD, int nIter, double dOverrideSigma, double& dMeanWeightedErrorSquared)
{
...
TooN::Vector<6> RelocaliserBruteForce::CalcPoseUpdate(TrackerDataPtrVector& vTD, double dOverrideSigma, double& dMeanWeightedErrorSquared)
{

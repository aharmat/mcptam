...
void VideoSourceSingle::InfoCallback(const sensor_msgs::CameraInfo::ConstPtr& infoMsg)
{
  ...
  {
    ROS_INFO_STREAM("VideoSourceSingle: Camera does not have a valid Taylor model calibration. Using set of default "
                    "values.");
  ...
  else
  {
    ...
    if (!mbGetPoseSeparately)
    {
      ...
      if (v3Diff * v3Diff > 1e-4)
      {
        ...
        ROS_WARN_STREAM("Perhaps you meant to get the camera pose separately but forgot to set the get_pose_separately "
                        "parameter to true?");
        ROS_WARN_STREAM("Setting the rotation matrix to identity!");
        ROS_WARN_STREAM("If you are calibrating the camera, continue without worry. If you are not, things will be "
                        "messed up.");

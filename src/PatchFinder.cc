...
PatchFinder::PatchFinder(int nPatchSize)
  : mimTemplate(CVD::ImageRef(nPatchSize,nPatchSize))
{
...
int PatchFinder::CalcSearchLevelAndWarpMatrix(MapPoint &point,
					      SE3<> se3CFromW,
...
void PatchFinder::MakeTemplateCoarse(MapPoint &point,
				     SE3<> se3CFromW,
				     Matrix<2> &m2CamDerivs)
{
...
void PatchFinder::MakeTemplateCoarseCont(MapPoint &point)
{
  ...
  if(bNeedToRefreshTemplate)
  {
    ...
    nOutside = CVD::transform(point.mpPatchSourceKF->maLevels[point.mnSourceLevel].image, 
                              mimTemplate, m2, vec(point.mirCenter), vec(mirCenter)); 
...
double PatchFinder::IterateSubPix(KeyFrame &kf)
{
  ...
  unsigned long nRowOffset = &kf.maLevels[mnSearchLevel].image[CVD::ImageRef(0,1)] - &kf.maLevels[mnSearchLevel].image[CVD::ImageRef(0,0)];
  for(ir.y = 1; ir.y < mnPatchSize - 1; ir.y++)
  {
    ...
    for(ir.x = 1; ir.x < mnPatchSize - 1; ir.x++)
    {
      ...
      float fPixel =  fMixTL * pTopLeftPixel[0] + fMixTR * pTopLeftPixel[1] +  

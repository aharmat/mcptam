...
KeyFrameVisualizer::KeyFrameVisualizer(Map &map, GLWindow2 &glw, ImageRefMap mDrawOffsets, ImageRefMap mSizes)
: mMap(map)
, mGLWindow(glw)
...
void KeyFrameVisualizer::Draw()
{
  mMessageForUser.str(""); // Wipe the user message clean
  
  // Lock the map while we draw to make sure nobody deletes anything from under our nose
  // This won't affect the Tracker since the KeyFrameVisualizer is called in the same thread,
...
std::string KeyFrameVisualizer::GetMessageForUser()
{

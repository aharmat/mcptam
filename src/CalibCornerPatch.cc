...
CalibCornerPatch::CalibCornerPatch(int nSideSize, CVD::ImageRef irDrawOffset)
...
void CalibCornerPatch::MakeTemplateWithCurrentParams()
{
  ...
  do
  {
    mimGradients[ir][0] = 0.5 * (imToBlur[ir + CVD::ImageRef(nOffset + 1, nOffset)] - imToBlur[ir + CVD::ImageRef(nOffset - 1, nOffset)]);
    mimGradients[ir][1] = 0.5 * (imToBlur[ir + CVD::ImageRef(nOffset, nOffset + 1 )] - imToBlur[ir + CVD::ImageRef(nOffset, nOffset - 1 )]);
  }
...
{ 
  Vector<2> v2TL = mParams.v2Pos - vec(mimTemplate.size() - CVD::ImageRef(1,1)) / 2.0;
  if(!(v2TL[0] >= 0.0 && v2TL[1] >= 0.0))
    return -1.0;
    
  Vector<2> v2BR = v2TL + vec(mimTemplate.size() - CVD::ImageRef(1,1));
  ...
  do
  {
    Vector<2> v2Pos_Template = vec(ir) - vec(mimTemplate.size() - CVD::ImageRef(1,1)) / 2.0;

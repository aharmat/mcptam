...
{  
  ...
  cv::Mat imgWrapped(kf.maLevels[0].image.size().y, kf.maLevels[0].image.size().x, CV_8U, kf.maLevels[0].image.data(), kf.maLevels[0].image.row_stride());
...
/*
void SmallBlurryImage::SE3fromSE2(SE2<> se2, TaylorCamera& cameraSrc, TaylorCamera& cameraTarget, TooN::SE3<>& se3, TooN::Matrix<6>& m6Cov) 
{
  // Do this by projecting two points, and then iterating the SE3<> (SO3
  // actually) until convergence. It might seem stupid doing this so
  // precisely when the whole SE2-finding is one big hack, but hey.

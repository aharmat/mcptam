...
#ifndef __MAP_MAKER_H
#define __MAP_MAKER_H

#include <mcptam/MapMakerClientBase.h>
#include <mcptam/MapMakerServerBase.h>
#include <cvd/thread.h>

/** @brief Implements standalone map maker by inheriting from both client-side and server-side code
 * 
...
public:
  ...
  /// Overridden from MapMakerClientBase, entry point for Tracker to initialize the map with one MultiKeyFrame containing multiple KeyFrames
...
protected:
  ...
  /// Overridden from MapMakerClientBase, called to add a MultiKeyFrame from the internal queue to the map
  virtual void AddMultiKeyFrameFromTopOfQueue();
  
  /// Overridden from MapMakerServerBase, deletes bad points and removes pointers to them from internal queues
  virtual void HandleBadEntities();
  
  ///Overridden from MapMakerServerBase, returns the size of the tracker queue as that is the data source for us.

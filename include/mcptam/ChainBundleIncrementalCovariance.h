...
/****************************************************************************************
 *
 * \file ChainBundleIncrementalCovariance.h
 * \brief Declaration of ChainBundleIncrementalCovariance class
 *
 * Copyright 2014   Adam Harmat, McGill University (adam.harmat@mail.mcgill.ca)
 *                  Michael Tribou, University of Waterloo (mjtribou@uwaterloo.ca)
 *
 * ChainBundleIncrementalCovariance has the same function as the Bundle class included with PTAM, but achieves
 * it quite differently. Whereas the Bundle class used a straight implementation of Hartley
 * and Zisserman's bundle adjustment code from the book "Multiple View Geometry",
 * ChainBundleIncrementalCovariance uses the G2O generic graph optimizer (http://openslam.org/g2o.html). 
 *
 * A single measurement is a function of a map point and an observing camera. However, 
 * unlike traditional BA, these are not specified in the world frame. Rather, they are
 * specified in the frame of their parent entity, whose pose is likewise specified in ITS
 * parent entity's frame, etc, forming pose chains. Therefore, each measurement has two
 * pose chains, one that defines the location of the map point, and one that defines the
 * location of the observing camera.
 * 
 * In mcptam, where MapPoints are defined relative to KeyFrames, which
 * belong to a MultiKeyFrame that is located in the world frame, we only have pose
 * chains of length 3 at most: world ===== MKF ===== KF ===== point, but pose chains could be of any length.
 * For example, they could implement Relative Bundle Adjustment (by Sibley et al.) where
 * the pose chains are very long. However, since the code was not designed with relative
 * bundle adjustment in mind, it might not be very efficient at performing that task.
 * Another example is during camera extrinsic calibration, where the pose chains share
 * the link that defines the relative pose between KFs so that there is only a single transformation 
 * being estimated between any two KFs. 
 *
 * The point features are parameterized with respect to their anchor KF and MKF.
 * Therefore, when giving a point to ChainBundleIncrementalCovariance it needs not only the 3-vector 
 * spherical coordinates for the point but also the source KeyFrame that the point is
 * defined in.
 *
 ****************************************************************************************/

#ifndef __CHAIN_BUNDLE_INCREMENTAL_COVARIANCE_H
#define __CHAIN_BUNDLE_INCREMENTAL_COVARIANCE_H
...
namespace g2o
{
  class SparseOptimizerIncrementalCovariance;
}
...
/** @brief Performs bundle adjustment on a chain of poses where the final pose is linked to map points through measurements.
 * 
 *  Unlike the Bundle class in PTAM, ChainBundleIncrementalCovariance supports multiple camera models and multiple measurements between
 *  one point and one camera (which is the movable base). 
 */
class ChainBundleIncrementalCovariance
{
public:
  ...
  ChainBundleIncrementalCovariance(TaylorCameraMap& cameraModels, bool bUseRobust, bool bUseTukey, bool bVerbose);   
  
  /// Destructor
  ~ChainBundleIncrementalCovariance();
   ...
   *  @return An index by which the pose is referenced inside ChainBundleIncrementalCovariance */
  int AddPose(TooN::SE3<> se3PoseFromRef, bool bFixed); 
  
  int AddPoseIncrementally(TooN::SE3<> se3PoseFromRef);
  
  /** @brief Add a map point to the system.
   *  @param v3PointInCam The coordinates of the point in the parent camera's frame
   ...
   *  @return An index by which the point is referenced inside ChainBundleIncrementalCovariance */
  ...
  void AddMeas(std::vector<int> vCams, int nPointIdx, TooN::Vector<2> v2Pos, double dNoiseSigmaSquared, std::string cameraName);
  
  void AddMeasIncrementally(std::vector<int> vCams, int nPointIdx, TooN::Vector<2> v2Pos, double dNoiseSigmaSquared, std::string cameraName);
  
  void WipeIncrementalMeas(){ msIncrementalEdges.clear(); }
   ...
   * @param dUserLambda User specified initial lambda value for Levenberg-Marquardt algorithm
   * @return Number of accepted update iterations, or negative on error. */
  int Compute(bool *pAbortSignal, int nNumIter = ChainBundleIncrementalCovariance::snMaxIterations, double dUserLambda=-1);  
  ...
  void SetPose(int n, TooN::SE3<> se3PoseFromRef); 
   ...
   * 
   * The measurements are encoded as a tuple of <point idx, pose idx, camera name>, which
  ...
  /** @brief Get the chi2 value (sum of errors squared weighted by information and robust kernel) */
  double GetMeanChiSquared();
  
  /** @brief Get the maximum covariance of the point feature depth */
  double GetMaxCov(){ return mdLastMaxCov; }
  ...
  static double sdUpdatePercentConvergenceLimit;   ///< If residual error changes less than this, consider problem converged
  static double sdUpdateRMSConvergenceLimit;    ///< If update magnitude RMS less than this, consider problem converged
  static double sdMinMEstimatorSigma;      ///< The minimum allowed sigma squared value to use with MEstimator
  
protected:
  
  g2o::SparseOptimizerIncrementalCovariance* mpOptimizer;                     ///< The actual optimizer
  RobustKernelData* mpRobustKernelData;                  ///< Data for the robust kernel
  
  // The following are "actions" that are added to the optimizer to be performed at various points in the minimization loop
  UpdateSigmaSquaredAction* mpUpdateSigmaSquaredAction;      ///< Update the sigma squared used for robustifying measurements
  ...
  /// A map of pose chain => helper class, used to calculate pose chain transforms only once per chain rather than inside the error function of 
  /// each measurement.
  HelperMap mmHelpers;  
  ...
  g2o::HyperGraph::EdgeSet msIncrementalEdges;

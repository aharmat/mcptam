 ...
 * 
 * PoseCalibrator derives from SystemBase and implements the main running code of the 
 * pose calibrator. It gets new images, feeds them to the CalibratorTrackers, and 
 * assembles new MultiKeyFrames from them when one wants to add to the Map.
 *
 ****************************************************************************************/

#ifndef __POSE_CALIBRATOR_H
#define __POSE_CALIBRATOR_H
...
/** @brief Inherits from SystemBase, implements the main running code of the pose calibrator */
...
private:
  ...
  /** @brief Take the KeyFrame from a TrackerCalib and put it into a MultiKeyFrame
   * 
   *  The TrackerCalib's current MultiKeyFrame will be deleted after this, so it should
   *  regenerate it. There was already a mechanism in place to regenerate MultiKeyFrames but 
   *  not KeyFrames, which is why I decided to just destroy the whole MultiKeyFrame instead of
   *  simply taking the KeyFrame.
   * 
   *  Note that this function needs to be called in the appropriate order, ie first call should
   *  be with the TrackerCalib corresponding to the first KeyFrame, etc. This is because
   *  the first KeyFrame will set up the MultiKeyFrame's pose, and the subsequent KeyFrames
   *  depend on this being set.
   *  @param pMKF Pointer to the MultiKeyFrame that will be the new owner of the KeyFrame
   ...
   * 
   *  Gets new images, feeds them to the CalibratorTrackers, decides which one should be
  ...
  /** @brief Publishes TF transforms for all the cameras relative to the frame vision_world */
  void PublishPoses();

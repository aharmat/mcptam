...
// intrusive pointer to increment/decrement the map point's and MKF's "using"
// counter.
//
//=========================================================================================

#ifndef MCPTAM_TRACKERDATA_H
#define MCPTAM_TRACKERDATA_H
...
/** @brief Contains all the intermediate results associated with a map-point
 *that the tracker keeps up-to-date.
 *
 * TrackerData basically handles all the tracker's point-projection jobs, and
 *also contains the PatchFinder which does t
 *  the image search.  */
class TrackerData
{
public:
   ...
   */
  TrackerData(MapPoint *pPoint, CVD::ImageRef irImageSize) : mPoint(*pPoint), mirImageSize(irImageSize) {}
  ...
  bool mbInImage;                ///< Is the projection of the point within the image
  /// boundaries?

  int mnSearchLevel;  ///< The pyramid level that the PatchFinder determined
  /// should hold the point
  ...
  TooN::Vector<2> mv2Error_CovScaled;  ///< The projection error scaled by the
  /// inverse noise term
   ...
   *
   * This can bail out at several stages if the point will not be properly in
   *the image.
   ...
   * results from the previous projection: Only do this right after the same
   ...
   *
   * This function returns the derivative of the pixel (x,y) location of a
   *projected
   * point  with respect to the six degrees of freedom that make up a camera's
   *pose.
   * In a multi-camera setup where the pose of a camera is fixed with respect to
   *a base
   * pose, but the base itself can move, getting the Jacobian is a bit trickier
   *than the
   * single camera case. In essence we need to calculate the derivative of the
   *projected point
   * with respect to the pose of the base, which is the only thing that can
   *move. See
   * source code for details.
   */
  inline void CalcJacobian(const TooN::SE3<> &se3BaseFromWorld, const TooN::SE3<> &se3CamFromBase)
  {
    const TooN::Vector<3> v3Cam =
        se3CamFromBase * se3BaseFromWorld * mPoint.mv3WorldPos;            // The point in the camera coordinate frame
    const TooN::Vector<3> v3Base = se3BaseFromWorld * mPoint.mv3WorldPos;  // The point in the base
                                                                           // (MultiKeyFrame) coordinate
                                                                           // frame
    const TooN::Vector<4> v4Base = unproject(v3Base);  // The SE3 generator field function works on 4-vectors

    TooN::SE3<> se3CamFromBase_only_rot = se3CamFromBase;  // Only the rotational component of the camera-from-base
                                                           // transform is needed
    ...
    for (int m = 0; m < 6; m++)
    {
      // Get the motion of the point in the base frame when the pose of the base
      // changes by one degree of freedom
      const TooN::Vector<4> v4Motion_Base = TooN::SE3<>::generator_field(m, v4Base);
      /* Then the motion of the point in the camera frame only depends on the
       * relative rotation between camera and base,
       * not the translation */
      const TooN::Vector<4> v4Motion_Cam = se3CamFromBase_only_rot * v4Motion_Base;
      ...
      /* The camera derivatives are motion of pixel relative to motion on unit
      sphere, so a multiplication gets the
      desired Jacobian entry */
      mm26Jacobian.T()[m] = mm2CamDerivs * v2CamSphereMotion;
    }
  }

  /// Allows updating the the error linearly instead of reprojecting the point,
  /// saves time
  ...
  /// Allows a TrackerData object accessed through a boost intrusive_ptr to
  /// increment the "using" count of its MapPoint
  ...
  /// Allows a TrackerData object accessed through a boost intrusive_ptr to
  /// decrement the "using" count of its MapPoint
